<template>
    <div>
        <v-btn color="primary" to="/admin/infrastructure/add">
            <v-icon>add</v-icon>
        ...
        <div>
            <data-table :headers='headers' :items='datas' :search.sync='cari'>
                <template slot="field" slot-scope="props">
                    ...
                    <td>
                        <v-tooltip left>
                            <v-btn fab dark small color ="cray" slot="activator" :to='"/admin/infrastructure/"+props.props.item.id+"/edit"'>
...
export default {
    ...
    methods: {
        del(val){
            console.log("hapus")
            api.deleteApi('/infrastructure',val.id)
                .then(
                    this.getAll()
                ).catch((ex)=>{
                    console.log(ex)
